...
export class OutboxRelayWorker implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  async onApplicationBootstrap(): Promise<void> {
    this.logger.info('Balance Outbox Relay Worker starting...');
    this.isRunning = true;
    this.scheduleNextPoll();
  }

  async onApplicationShutdown(): Promise<void> {
    this.logger.info('Balance Outbox Relay Worker shutting down...');
  ...
  async processBatch(): Promise<void> {
    ...
    for (const event of events) {
      ...
      try {
        ...
        // aggregateId represents the merchantBalanceId/partition key
