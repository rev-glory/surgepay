...
async function bootstrap(): Promise<void> {
  const loggingConfig: LoggingConfig = {
    ...
    serviceName: process.env.LOG_SERVICE_NAME || 'payment-service',
  };

  const bootstrapLogger = LoggerFactory.createStandaloneLogger(loggingConfig);
  bootstrapLogger.info('Bootstrapping Payment Service...');
  ...
  // Use port 3003 for Payment Service by default
  ...
  logger.info(`Payment Service started successfully on http://${host}:${port}/${prefix}/v${defaultVersion}`);
}

bootstrap().catch((error) => {
  console.error('Fatal error during Payment Service bootstrapping:', error);
  process.exit(1);
});
