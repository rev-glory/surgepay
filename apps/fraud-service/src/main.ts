...
async function bootstrap(): Promise<void> {
  const loggingConfig: LoggingConfig = {
    ...
    serviceName: process.env.LOG_SERVICE_NAME || 'fraud-service',
  };

  const bootstrapLogger = LoggerFactory.createStandaloneLogger(loggingConfig);
  bootstrapLogger.info('Bootstrapping Fraud Pre-check Service...');
  ...
  // Use port 3008 for Fraud Service by default
  ...
  logger.info(`Fraud Pre-check Service started successfully on http://${host}:${port}/${prefix}/v${defaultVersion}`);
}

bootstrap().catch((error) => {
  console.error('Fatal error during Fraud Pre-check Service bootstrapping:', error);
  process.exit(1);
});
