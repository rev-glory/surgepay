...
async function bootstrap(): Promise<void> {
  const loggingConfig: LoggingConfig = {
    ...
    serviceName: process.env.LOG_SERVICE_NAME || 'retry-scheduler',
  };

  const bootstrapLogger = LoggerFactory.createStandaloneLogger(loggingConfig);
  bootstrapLogger.info('Bootstrapping Retry Scheduler...');
  ...
  // Use port 3012 for Retry Scheduler Service by default
  ...
  logger.info(`Retry Scheduler started successfully on http://${host}:${port}/${prefix}/v${defaultVersion}`);
}

bootstrap().catch((error) => {
  console.error('Fatal error during Retry Scheduler bootstrapping:', error);
  process.exit(1);
});
