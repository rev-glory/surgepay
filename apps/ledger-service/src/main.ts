...
async function bootstrap(): Promise<void> {
  const loggingConfig: LoggingConfig = {
    ...
    serviceName: process.env.LOG_SERVICE_NAME || 'ledger-service',
  };

  const bootstrapLogger = LoggerFactory.createStandaloneLogger(loggingConfig);
  bootstrapLogger.info('Bootstrapping Ledger Service...');
  ...
  // Use port 3008 for Ledger Service by default
  ...
  logger.info(`Ledger Service started successfully on http://${host}:${port}/${prefix}/v${defaultVersion}`);
}

bootstrap().catch((error) => {
  console.error('Fatal error during Ledger Service bootstrapping:', error);
  process.exit(1);
});
