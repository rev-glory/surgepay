...
async function bootstrap(): Promise<void> {
  const loggingConfig: LoggingConfig = {
    ...
    serviceName: process.env.LOG_SERVICE_NAME || 'order-service',
  };

  const bootstrapLogger = LoggerFactory.createStandaloneLogger(loggingConfig);
  bootstrapLogger.info('Bootstrapping Order Service...');
  ...
  // Use port 3004 for Order Service by default
  ...
  logger.info(`Order Service started successfully on http://${host}:${port}/${prefix}/v${defaultVersion}`);
}

bootstrap().catch((error) => {
  console.error('Fatal error during Order Service bootstrapping:', error);
  process.exit(1);
});
