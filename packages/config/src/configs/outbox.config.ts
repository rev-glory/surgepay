...
export default registerAs('outbox', (): OutboxConfig => ({
  pollingInterval: process.env.OUTBOX_POLLING_INTERVAL
    ...
    : 500,
  batchSize: process.env.OUTBOX_BATCH_SIZE ? parseInt(process.env.OUTBOX_BATCH_SIZE, 10) : 100,
  retryLimit: process.env.OUTBOX_RETRY_LIMIT ? parseInt(process.env.OUTBOX_RETRY_LIMIT, 10) : 3,
